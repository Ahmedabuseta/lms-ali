...
export async function canAccessNextChapter(
  ...
  currentChapterPosition: number
    ...
    if (previousChapterQuiz && previousChapterQuiz.attempts.length > 0) { // If there's a quiz, check if the user has passed it
  ...
  } catch (error) { console.error('Error checking chapter access:', error);
    return false; }
}
...
export async function getChapterAccessInfo(
  userId: string,
  courseId: string
): Promise<Record<string, boolean>> { try {
    ...
    for (const chapter of chapters) { accessInfo[chapter.id] = await canAccessNextChapter(
        userId,
        courseId,
        chapter.position
      ); }
    ...
    return accessInfo;
  } catch (error) { console.error('Error getting chapter access info:', error);
    return { };
  }
}
