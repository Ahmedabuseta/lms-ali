...
export async function POST(req: Request, { params }: { params: { courseId: string } }) { try {
      requireTeacher()
  ...
  } catch (error) { console.error('[FLASHCARDS_POST]', error);
    return new NextResponse('Internal Error', { status: 500 });
  }
}

export async function GET(req: Request, { params }: { params: { courseId: string } }) { try {
requireAuth()
  ...
  } catch (error) { console.error('[FLASHCARDS_GET]', error);
    return new NextResponse('Internal Error', { status: 500 });
  }
}
