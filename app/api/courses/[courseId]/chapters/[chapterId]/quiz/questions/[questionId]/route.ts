...
export async function PATCH(
  ...
  try {
    ...
    if (quiz.isPublished) {
      return new NextResponse('Cannot edit questions in published quiz', { status: 400 });
    }
    ...
    if (!quizQuestion) {
      return new NextResponse('Question not found in quiz', { status: 404 });
    }
    ...
    console.error('[QUIZ_QUESTION_UPDATE]', error);
