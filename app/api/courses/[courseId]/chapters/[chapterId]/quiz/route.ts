...
export async function GET(req: NextRequest, { params }: { params: Params }) { try {
  ...
  } catch (error) { console.error('[CHAPTER_QUIZ_GET]', error);
    return new NextResponse('Internal Error', { status: 500 });
  }
}

export async function POST(req: NextRequest, { params }: { params: Params }) { try {
    ...
    const chapter = await db.chapter.findUnique({ where: {
        id: params.chapterId,
        courseId: params.courseId, },
    });
    ...
    const existingQuiz = await db.quiz.findFirst({ where: { chapterId: params.chapterId },
    });

    if (existingQuiz) { return new NextResponse('Quiz already exists for this chapter', { status: 400 });
    }
    ...
    return NextResponse.json(quiz);
  } catch (error) { console.error('[CHAPTER_QUIZ_CREATE]', error);
    return new NextResponse('Internal Error', { status: 500 });
  }
}
...
export async function PATCH(req: NextRequest, { params }: { params: Params }) { try {
    ...
    // Find existing quiz
    const existingQuiz = await db.quiz.findFirst({ where: {
        chapterId: params.chapterId,
        chapter: {
          courseId: params.courseId, },
      },
    ...
    if (!existingQuiz) { return new NextResponse('Quiz not found', { status: 404 });
    }

    // Update quiz
    const quiz = await db.quiz.update({ where: { id: existingQuiz.id },
      data: values,
    });

    return NextResponse.json(quiz);
  } catch (error) { console.error('[CHAPTER_QUIZ_UPDATE]', error);
    return new NextResponse('Internal Error', { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: Params }) { try {
  ...
  } catch (error) { console.error('[CHAPTER_QUIZ_DELETE]', error);
    return new NextResponse('Internal Error', { status: 500 });
  }
}
