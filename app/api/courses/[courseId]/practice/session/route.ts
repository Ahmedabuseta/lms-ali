...
export async function POST(
  ...
  try {
    ...
    if (!chapterIds || !Array.isArray(chapterIds) || chapterIds.length === 0) {
      return new NextResponse('Chapter IDs are required', { status: 400 });
    }
    ...
    if (chapters.length !== chapterIds.length) {
      return new NextResponse('Invalid chapter selection', { status: 400 });
    }
    ...
    const questions = await db.question.findMany({
      ...
      orderBy: {
        createdAt: 'desc', // You can randomize this later
      },
    });
    ...
    const previousAttempts = await db.practiceAttempt.findMany({
      where: {
        userId: user.id,
    ...
    const questionsWithHistory = questions.map(question => {
      const attempts = previousAttempts.filter(attempt => attempt.questionId === question.id);
    ...
    return NextResponse.json({
      sessionId: `practice_${mode}_${Date.now()}_${user.id}`,
      courseId: params.courseId,
      mode: mode,
    ...
    console.error('[PRACTICE_SESSION_POST]', error);
