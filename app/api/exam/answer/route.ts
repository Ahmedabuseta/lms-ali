...
const submitAnswerSchema = z.object({ attemptId: z.string().min(1, 'معرف المحاولة مطلوب'),
...
export async function POST(req: Request) { try {
    ...
    // Submit the answer
    const questionAttempt = await submitAnswer({ userId,
      attemptId,
      questionId,
      optionId, });
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('time has expired') || error.message.includes('Time limit exceeded')) { return NextResponse.json(
          {
      ...
      if (error.message.includes('Invalid option')) { return NextResponse.json(
          {
