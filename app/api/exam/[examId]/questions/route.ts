...
const addQuestionSchema = z.object({
  questionBankId: z.string().min(1, 'معرف بنك الأسئلة مطلوب'),
...
export async function GET(req: Request, { params }: { params: { examId: string } }) {
  try {
    ...
    console.error('[GET_EXAM_QUESTIONS_ERROR]', error);
    return NextResponse.json(
...
export async function POST(req: Request, { params }: { params: { examId: string } }) {
  try {
    ...
    if (action === 'add_existing') {
      ...
      return NextResponse.json({
        message: 'تم إنشاء السؤال وإضافته بنجاح',
        examQuestion,
        question,
      });
    ...
    console.error('[ADD_EXAM_QUESTION_ERROR]', error);
