...
interface AttemptRequest { sessionId: string;
  questionId: string;
...
export async function POST(req: NextRequest) { try {
    ...
    const { sessionId,
      ...
      selectedOptionId,
      isCorrect,
      timeSpent = 0 }: AttemptRequest = await req.json();

    if (!sessionId || !questionId || !selectedOptionId) { return new NextResponse('Missing required fields', { status: 400 });
    }
    ...
    if (isCorrect !== actualIsCorrect) { return new NextResponse('Correctness mismatch', { status: 400 });
    }
    ...
    const result = { id: practiceAttempt.id,
      isCorrect: actualIsCorrect,
  ...
  } catch (error) { console.log('[PRACTICE_ATTEMPT_CREATE]', error);
...
export async function GET(req: NextRequest) { try {
    ...
    if (!questionId) { return new NextResponse('Question ID required', { status: 400 });
    }

    const attempts = await db.practiceAttempt.findMany({ where: {
        userId: user.id,
        questionId: questionId },
    ...
    return NextResponse.json({ attempts,
      stats });
  } catch (error) { console.log('[PRACTICE_ATTEMPT_GET]', error);
    return new NextResponse('Internal Error', { status: 500 });
  }
}
