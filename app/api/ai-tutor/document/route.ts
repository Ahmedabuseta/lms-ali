...
// Function to extract text from an image URL using OCR
...
export async function POST(req: Request) {
  try {
      ...
      const extractionResults = await Promise.all(
        documentFiles.map(async (file) => {
          try {
            ...
            processingErrors.push(`Error processing ${file.name}: ${error instanceof Error ? error.message : 'Unknown error'}`);
            return {
              success: false,
              fileName: file.name,
    ...
    const systemMessage = {
      ...
      content:
        'You are a helpful AI tutor. When provided with text extracted from documents, analyze the content carefully and answer questions about it. Explain concepts thoroughly and clearly.',
    };
    ...
    payload.messages = [...payload.messages, ...messages];
