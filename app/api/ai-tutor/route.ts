...
export async function POST(req: Request) {
  try {
    ...
    const { messages, streaming = false, image } = body as { 
      messages: Message[],
      streaming?: boolean,
      image?: ImageData
    };
    ...
    if (image) {
      ...
      if (!messages[0] || messages[0].role !== 'system') {
        payload.messages = [
          {
            role: 'system',
            content: "You are an AI tutor that specializes in explaining concepts from images. When a user uploads an image, analyze it carefully and provide detailed explanations based on the image content and the user's question. For educational materials, explain the concepts thoroughly."
          },
          ...messages
        ];
      }
