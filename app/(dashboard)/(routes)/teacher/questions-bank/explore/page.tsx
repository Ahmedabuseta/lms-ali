...
interface ExploreQuestionsPageProps { searchParams: {
    page?: string;
    courseId?: string;
    type?: string;
    difficulty?: string;
...
const ExploreQuestionsPage = async ({ searchParams }: ExploreQuestionsPageProps) => { await requireAuth();
  ...
  if (searchParams.courseId && searchParams.courseId.trim() !== '') { filters.questionBank = {
  ...
  if (searchParams.search && searchParams.search.trim() !== '') { filters.text = {
  ...
  const [questions, totalQuestions, courses] = await Promise.all([
    db.question.findMany({ where: filters,
      include: {
        options: true,
        questionBank: {
          include: {
