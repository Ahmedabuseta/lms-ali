...
export default async function ExamPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="p-6 space-y-6" dir="rtl">
      <div>
        <h1 className="text-2xl font-bold text-right text-slate-900 dark:text-slate-100">الامتحانات</h1>
        <p className="text-sm text-slate-600 dark:text-slate-400 text-right">اختبر معلوماتك مع امتحانات المقررات بتاعتك</p>
      </div>

      <div className="space-y-6">
        {/* Course Filter Cards */}
        <div>
          <h2 className="text-lg font-medium mb-3 text-right text-slate-900 dark:text-slate-100">تصفية حسب الكورس</h2>
          <CourseCards 
            courses={courses}
            currentCourseId={searchParams.courseId}
          />
        </div>
        ...
        <div className="space-y-4">
          {exams.length === 0 ? (
            <div className="flex h-60 w-full flex-col items-center justify-center rounded-md border border-dashed border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-8 text-center">
