    ...
    <Card className="relative">
      <CardHeader>
        <CardTitle className="flex items-center justify-between flex-row-reverse">
          <span className="text-right">سؤال {currentQuestionIndex + 1} من {totalQuestions}</span>
          {showSavedIndicator && (
            <span className="text-xs text-green-600 dark:text-green-400 animate-pulse">تم حفظ الإجابة ✓</span>
          )}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        >
          {question.options.map((option) => (
            <div 
              key={option.id} 
              className={`flex items-center space-x-2 space-x-reverse p-3 rounded-md border transition-colors ${
            ...
            >
              ...
              <Label htmlFor={option.id} className="cursor-pointer text-right flex-grow">
        ...
        {isSubmitting && (
          <div className="flex justify-center items-center py-2">
            ...
            <span className="text-xs text-muted-foreground mr-2">جاري الحفظ...</span>
          </div>
        )}
      </CardContent>
