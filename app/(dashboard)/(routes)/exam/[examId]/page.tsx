    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div>
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-right text-slate-900 dark:text-slate-100">{exam?.title}</h1>
          {exam?.timeLimit && (
            <div className="flex items-center rounded-md bg-slate-100 dark:bg-slate-800 px-3 py-1 text-sm text-slate-700 dark:text-slate-300">
      ...
      <div className="space-y-4">
        {pastAttempts && pastAttempts.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-xl text-right text-slate-900 dark:text-slate-100">
                المحاولات السابقة
              </CardTitle>
            ...
            <CardContent>
              <div className="space-y-2">
                {pastAttempts.map((attempt) => (
                  <div key={attempt.id} className="flex flex-row-reverse items-center justify-between rounded-md border border-slate-200 dark:border-slate-800 p-3">
                    <div className="flex flex-row-reverse items-center space-x-4 space-x-reverse">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400">
                          أكملت في {attempt?.completedAt ? new Date(attempt.completedAt).toLocaleDateString('ar-SA') : 'غير متاح'}
                        </p>
                      </div>
                    </div>
                    <Button variant="outline" size="sm" asChild>
                      <a href={`/exam/${exam.id}/results/${attempt.id}`}>
                        عرض النتائج
                      </a>
                    </Button>
                  </div>
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="p-6">
            {exam?.description && (
              <div className="mb-4 text-sm text-right text-slate-600 dark:text-slate-400">
                {exam?.description}
              </div>
            )}
            <div className="rounded-md bg-slate-50 dark:bg-slate-800/50 p-4">
              <h3 className="font-medium text-right text-slate-900 dark:text-slate-100">تعليمات الامتحان</h3>
              <ul className="mt-2 list-inside list-disc space-y-1 text-sm text-slate-600 dark:text-slate-400 text-right">
                ...
                {exam?.timeLimit && (
                  <li>عندك {exam?.timeLimit} دقيقة لإنهاء الامتحان</li>
                )}
              </ul>
            </div>
          </CardContent>
          <CardFooter className="flex justify-between flex-row-reverse bg-slate-50 dark:bg-slate-800/50 p-6">
