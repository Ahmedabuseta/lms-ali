...
interface FlashcardProps { id: string;
  question: string;
  answer: string; }

interface FlashcardClientProps { initialCards: FlashcardProps[];
  courseId?: string;
  chapterId?: string; }

export default function FlashcardClient({ initialCards, courseId, chapterId }: FlashcardClientProps) { // Using state initialized from props to avoid hydration mismatches
  ...
  const loadMoreCards = async (page: number = 1): Promise<{ flashcards: FlashcardProps[], hasNextPage: boolean, totalCount: number }> => { try {
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      } else { setCards((prevCards) => [...prevCards, ...result.flashcards]);
    ...
    } catch (error) { console.error('Failed to load more cards:', error);
      toast.error('حدث خطأ أثناء تحميل البطاقات'); } finally {
  ...
  const handleRandomize = () => { if (cards.length <= 1) return;
  ...
  const bind = useDrag(({ down, movement: [_mx, my], direction: [_dx, dy], velocity, cancel }) => {
    ...
    if (down) { api.start({
        ...
        scale: 1.02,
        immediate: true, });
    } else { // Swipe threshold - if velocity is high enough or movement is great enough
          ...
          onRest: () => {
            ...
            setCurrentIndex((current) => {
              if (isSwipeUp) {
                ...
                return current === cards.length - 1 ? 0 : current + 1; } else { // When swiping down (previous card)
      ...
      } else { // Return card to original position if not swiped enough
        api.start({
          x: 0,
          y: 0,
          rotation: 0,
  ...
  if (cards.length === 0 && initialCards.length > 0) { return (
