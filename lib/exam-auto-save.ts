...
export class ExamAutoSaveService {
  private static instance: ExamAutoSaveService;
  ...
  private async saveAnswerToServer(answer: ExamAnswer): Promise<void> {
    ...
    try {
      ...
      if (shouldRetry) {
        ...
        const retryTimeout = setTimeout(() => {
          this.performSave(answer, retryCount + 1);
        }, retryDelay);
