...
const validateEnvironment = () => { const required = {
...
const initializeSpaces = () => { try {
    ...
    console.log('Initializing Digital Ocean Spaces with:', { endpoint,
  ...
  } catch (error) { console.error('Failed to initialize Digital Ocean Spaces:', error);
    throw error; }
};
...
export const uploadToSpaces = async (
  ...
  folder: string = ''
    ...
    const params: AWS.S3.PutObjectRequest = { Bucket: bucket,
      Key: uniqueFileName,
      Body: file,
      ACL: 'public-read',
    ...
    console.log('Upload successful:', { location: data.Location,
      etag: data.ETag,
      key: data.Key, });

    return data.Location;
  } catch (error) { console.error('Error uploading to DigitalOcean Spaces:', error);

    // Provide more specific error messages
    if (error instanceof Error) {
      if (error.message.includes('SignatureDoesNotMatch')) {
        throw new Error('Invalid Digital Ocean Spaces credentials. Please check your access key and secret.'); } else if (error.message.includes('InvalidAccessKeyId')) {
...
export const deleteFromSpaces = async (fileUrl: string) => { try {
    const { s3, bucket } = getSpacesConfig();
    ...
    return true;
  } catch (error) { console.error('Error deleting from DigitalOcean Spaces:', error);
    throw new Error('Failed to delete file from Digital Ocean Spaces.'); }
};

export const getSignedUrl = (key: string, expires: number = 3600) => { try {
    const { s3, bucket } = getSpacesConfig();
...
export const testSpacesConnection = async () => { try {
    const { s3, bucket } = getSpacesConfig();
    ...
    return true;
  } catch (error) { console.error('❌ Digital Ocean Spaces connection failed:', error);
    return false; }
};
