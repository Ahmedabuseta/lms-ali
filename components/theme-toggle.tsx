...
export function ThemeToggle() { const { setTheme, theme, resolvedTheme } = useTheme();
  ...
  const toggleTheme = () => { // Simple toggle between light and dark
      ...
      <div className="relative">
        ...
        <Moon
          className={ `absolute inset-0 h-5 w-5 transform transition-all duration-500 ${
            isDark ? 'rotate-0 scale-100 opacity-100' : '-rotate-90 scale-0 opacity-0' } text-indigo-400`}
        />
      </div>
