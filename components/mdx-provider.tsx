  ...
  return (
    <div className="my-4 border rounded-md">
      <button
        ...
        className="flex justify-between w-full p-4 text-left font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
      >
...
const components = {
  ...
  pre: ({ className, ...props }: React.HTMLAttributes<HTMLPreElement>) => (
    <pre
      className={cn(
        "mb-4 mt-6 overflow-x-auto rounded-lg border bg-slate-950 p-4",
        className
      )}
      {...props}
    />
...
export function MDXContent({ children }: { children: ReactNode }) {
  return (
    <MDXProvider components={components}>
      <div className="mdx-content prose prose-slate max-w-none dark:prose-invert prose-headings:font-bold prose-headings:tracking-tight prose-lead:text-slate-500 prose-a:font-semibold prose-a:text-primary hover:prose-a:text-primary/80">
        {children}
      </div>
    </MDXProvider>
  );
