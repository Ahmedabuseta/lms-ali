...
type StudentResult = { userId: string;
  userName: string;
  email: string;
  score: number;
  timeTaken: number;
...
interface StudentPerformanceTableProps { studentResults: StudentResult[]; }

type SortField = 'userName' | 'score' | 'timeTaken' | 'completedAt';
type SortDirection = 'asc' | 'desc';

export function StudentPerformanceTable({ studentResults }: StudentPerformanceTableProps) { const [sortField, setSortField] = useState<SortField>('score');
  ...
  const sortedResults = [...studentResults].sort((a, b) => { if (sortField === 'userName') {
      return sortDirection === 'asc' ? a.userName.localeCompare(b.userName) : b.userName.localeCompare(a.userName); }

    if (sortField === 'score') { return sortDirection === 'asc' ? a.score - b.score : b.score - a.score; }

    if (sortField === 'timeTaken') { return sortDirection === 'asc' ? a.timeTaken - b.timeTaken : b.timeTaken - a.timeTaken; }

    if (sortField === 'completedAt') { return sortDirection === 'asc'
