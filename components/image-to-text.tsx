    ...
    <>
      <Card className="w-full">
          ...
          >
            {!state.imagePreview ? (
              <>
                ...
                <p className="text-xs text-muted-foreground">Supports JPG, PNG and GIF files (max 5MB)</p>
              </>
            ) : showPreview ? (
              <div className="relative w-full">
                ...
                {state.isProcessing && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/50 backdrop-blur-sm">
                    ...
                    <p className="text-sm font-medium">{state.ocrStatus || 'Processing image...'}</p>
                  </div>
                )}
              </div>
            ) : (
              <div className="flex items-center gap-2 py-2">
                ...
                {state.isProcessing && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}
              </div>
            )}
        ...
        <CardFooter className="flex justify-between gap-2">
          <Button variant="outline" disabled={state.isProcessing || !state.imagePreview} onClick={processImage}>
            <RefreshCw className="mr-2 h-4 w-4" />
            Re-process
          </Button>
          <Button variant="outline" disabled={state.isProcessing} onClick={handleReset}>
            <RefreshCw className="mr-2 h-4 w-4" />
            New Image
          </Button>
        </CardFooter>
      ...
      >
        <DialogContent className="flex h-[90vh] w-[90vw] max-w-screen-lg items-center justify-center p-0">
          {state.imagePreview && (
            <img src={state.imagePreview} alt="Full size preview" className="max-h-full max-w-full object-contain" />
          )}
        </DialogContent>
      </Dialog>
    </>
