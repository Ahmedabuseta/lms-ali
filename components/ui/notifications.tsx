...
export const useNotificationStore = create<NotificationStore>((set) => ({ notifications: [],
  position: 'top-right',
  addNotification: (notification) => {
    ...
    if (notification.duration) { setTimeout(() => {
        set((state) => ({
...
const notificationVariants = cva('relative flex gap-3 overflow-hidden rounded-lg border p-4 pr-12 shadow-lg', { variants: {
    type: {
...
interface NotificationProps extends React.HTMLAttributes<HTMLDivElement>, VariantProps<typeof notificationVariants> { notification: Notification;
  onClose: () => void; }

export function NotificationToast({ notification, onClose, type, className, ...props }: NotificationProps) { // Get icon component based on notification type and specified icon
  const getIcon = () => {
...
export function NotificationsContainer() { const { notifications, removeNotification, position } = useNotificationStore();
...
export const showNotification = { success: (title: string, description?: string, options?: Partial<Notification>) => {
  ...
  error: (title: string, description?: string, options?: Partial<Notification>) => { return useNotificationStore.getState().addNotification({
      title,
      description,
      type: 'error',
  ...
  info: (title: string, description?: string, options?: Partial<Notification>) => { return useNotificationStore.getState().addNotification({
      title,
      description,
      type: 'info',
      duration: 5000,
  ...
  warning: (title: string, description?: string, options?: Partial<Notification>) => { return useNotificationStore.getState().addNotification({
      title,
      description,
      type: 'warning',
      duration: 7000,
  ...
  achievement: (title: string, description?: string, options?: Partial<Notification>) => { return useNotificationStore.getState().addNotification({
      title,
      description,
      type: 'achievement',
